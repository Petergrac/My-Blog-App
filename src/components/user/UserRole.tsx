...
const UserRole = () => {
  ...
  if (!isLoaded) {
    ...
    return (
      <div className="flex flex-col items-center justify-center p-8 rounded-lg shadow-md max-w-sm mx-auto animate-pulse">
        <div className="w-10 h-10 rounded-full  mb-4"></div>
        <div className="h-4  rounded w-3/4 mb-2"></div>
        <div className="h-3  rounded w-1/2 mb-6"></div>
        <div className="h-10 rounded w-48"></div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col  items-center justify-center p-8 rounded-lg shadow-md max-w-sm mx-auto transform transition-all duration-300 hover:shadow-lg">
      ...
      <p className="text-gray-100 mb-6 text-center text-sm md:text-base">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger
          className="flex items-center gap-2 border  py-2 px-4 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed"
          disabled={isUpdating}
        >
          {isUpdating ? (
            ...
            <>
              <span className="font-medium text-base">
                Change Role ({currentRole})
              </span>
              <ChevronDownIcon className="w-4 h-4 ml-2 text-gray-500" />
            </>
          )}
        </DropdownMenuTrigger>
        <DropdownMenuContent className="w-(--radix-dropdown-menu-trigger-width)  shadow-lg rounded-lg border border-gray-600 mt-2">
          <DropdownMenuItem
            className="flex items-center justify-between px-4 py-2 hover:text-blue-600 cursor-pointer transition-colors duration-150 rounded-md"
