...
export const handleImageUpload = async (
  ...
  if (file.size > MAX_FILE_SIZE) {
    toast.error("FILE SIZE EXCEED MAX UPLOAD");
  }
  ...
  */
  ...
  const { data, error } = await supabase.storage
    .from("blog-post")
    .upload(`post-images/${newFile.name}`, newFile, { upsert: false });
  // Fake uploader simulator
  for (let progress = 0; progress <= 100; progress += 15) {
    await new Promise((resolve) => {
      setTimeout(() => {
        resolve(onProgress?.({ progress }));
      }, 500);
    });
  }
  if (error) {
  ...
  if (data) {
    ...
    const res = supabase.storage
      .from("blog-post")
      .getPublicUrl(`post-images/${newFile.name}`);
